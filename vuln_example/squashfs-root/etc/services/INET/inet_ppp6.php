...
function pppoptions($inf, $ifname, $profile, $default, $mode)
{
    ...
    /*fwrite("a",$optfile, "ipcp-accept-remote ipcp-accept-local +ipv6 ipv6cp-use-persistent ipv6cp-accept-local ipv6cp-use-ipaddr\n"); */
...
$optfile = pppoptions($inf, $ifname, $inetp."/ppp6", $default, $dial);
...
if($child != "")
{
    ...
    fwrite("a",$START, 'event CHILD.'.$child.'.UP add "service INFSVCS.'.$child.' start"\n');
    fwrite("a",$START, 'event CHILD.'.$child.'.DOWN add "service INFSVCS.'.$child.' stop"\n');
    fwrite("a",$START, 'xmldbc -s '.$stsp.'/uid  "'.$child.'"\n');
    fwrite("a",$START, 'xmldbc -s '.$stsp.'/parent  "'.$inf.'"\n');
    fwrite("a",$START, 'xmldbc -s '.$stsp.'/active  "1"\n');
    fwrite("a",$START, 'xmldbc -s '.$stsp.'/phyinf  "'.$phyinf.'"\n');
}
?>
