...
function check_inf($entry)
{
	...
	/*sandy, 2010/07/13 */
	/* check dhcpdetect */
	$value = query("dhcpdetect");
	TRACE_debug("FATLADY: INF: dhcpdetect=".$value);
	if ($value!="" && XNODE_getpathbytarget("", "inf", "uid", $value, 0)=="")
	{
		/* internal error, no i18n(). */
		set_result("FAILED", $entry."/dhcpdetect", "dhcpdetect=".$value." doesn't exit!");
		return 9;
	}
	/* check schedule */
	...
	/* check child */
	$value = query("child");
	TRACE_debug("FATLADY: INF: child=".$value);

	/* check 6TO4 */
	$value = query($entry."/inf6to4/mode");
	TRACE_debug("FATLADY: INF: 6to4 mode=".$value);
	if ($value!="")
	{
		$ipaddr = query($entry."/inf6to4/ipaddr");
		$prefix = query($entry."/inf6to4/prefix");
		$relay = query($entry."/inf6to4/relay");
		TRACE_debug("FATLADY: INF: 6to4 ipaddr=".$ipaddr);
		TRACE_debug("FATLADY: INF: 6to4 prefix=".$prefix);
		...
		{	
			...
			{		
				return set_result("FAILED", $entry."/inf6to4/ipaddr", i18n("Invalid IPv6 address"));
			}

			if ($prefix == "")
			{
				return set_result("FAILED", $entry."/inf6to4/prefix", i18n("No prefix value"));
			}		
			if (isdigit($prefix) == 0)
			{
				return set_result("FAILED", $entry."/inf6to4/prefix", i18n("Prefix value must be digit number"));
			}	
			if ($prefix <= 0 || prefix > 128)
			{
				return set_result("FAILED", $entry."/inf6to4/prefix", i18n("Invalid prefix value"));
			}	
		}
		if ($relay != "")
		{	
			...
			{
				return set_result("FAILED", $entry."/inf6to4/relay", i18n("Invalid IPv4 address"));
			}
		}	
	}
...
function check_networkid($inetp, $inf, $prefix)
{
	if (query($inetp."/ipv4/static")==1)
	{
		...
		foreach ($infp)
		{
			...
			/* The value of should not be 0. */
			if (query($inetp."/ipv4/mask")=="0")
				return set_result("FAILED", $inetp."/ipv4/mask", i18n("Invalid Subnet Mask value"));
			if (query($v_inetp."/ipv4/mask")=="0")
...
function fatlady_inet($prefix, $inf, $needgw)
{
	...
	if (query($infp."/active") == "1")
	{
		...
		else if	($addrtype == "ppp6") dophp("load", $b."/inet_ppp6.php");
